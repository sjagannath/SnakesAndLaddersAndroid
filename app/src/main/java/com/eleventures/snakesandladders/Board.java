...
public class Board {
	...
	private final HashMap<Integer, Obstacle> mObstacles = new HashMap<>();
	...
	public PlayTurnResult playTurn(int current, int diceRoll) {
		int tempPos = (current + diceRoll) > END ? END : (current + diceRoll);
		String message = "";
		if(tempPos >= END){
			message = "Congratulations! Winner winner chicken dinner!";
			tempPos = END;
		}else if (mObstacles.containsKey(tempPos)) {
			Obstacle obstacle = mObstacles.get(tempPos);
			tempPos = obstacle.mEndsAtPosition;
			message = obstacle.getMessage();
		}else{
			message = "You have rolled a " + diceRoll + " to land at " + tempPos;
		}
		return new PlayTurnResult(tempPos, tempPos >= END, message);
	}
}
